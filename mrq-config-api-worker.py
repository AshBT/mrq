...
MONGODB_LOGS = "mongodb://127.0.0.1:27017/mrq_logs"  # 1  # MongoDB URI for the logs database."0" will disable remote logs, "1" will use main MongoDB.Defaults to 1
...
""" mrq-worker settings
"""
QUEUES = ("api_worker",)  # The queues to listen on.Defaults to default , which will listen on all queues.
MAX_JOBS = 0  # Gevent:max number of jobs to do before quitting. Workaround for memory leaks in your tasks. Defaults to 0
# Max memory (in Mb) after which the process will be shut down. Use with PROCESS = [1-N] to have supervisord automatically respawn the worker when this happens.Defaults to 1
MAX_MEMORY = 50
...
DASHBOARD_QUEUE = "api_worker"  # Default queue for dashboard actions.
